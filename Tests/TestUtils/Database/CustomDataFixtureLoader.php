...
/**
 * Because doctrine loads all the files before PHPUnit can get them,
 * we have to filter out all the files except for the paths with a Fixture string in them
 *
 * Class CustomDataFixtureLoader
 * @package ASWatsonKKS\AdminBundle\Tests\TestUtils
 */
class CustomDataFixtureLoader {
    /**
    ...
    private function loadFromIterator(\Iterator $iterator)
    {
        ...
        foreach ($iterator as $file) {
            ...
            // ------------------- CHANGED -------------------------
            // -----------------------------------------------------
            if( strpos( $sourceFile, "Fixture" ) !== false
                && strpos( $sourceFile, "CustomDataFixtureLoader" ) === false ){
